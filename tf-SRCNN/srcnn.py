...
conv1 = tf.nn.relu(tf.nn.conv2d(input=inputs,filter=weights['w1'],strides=[1,1,1,1],padding="VALID")+biases['b1'])
##------ Add your code here: to compute non-linear mapping
# conv2 layer with biases and relu: 32 filters with size 1 x 1

conv2 = tf.nn.relu(tf.nn.conv2d(input=conv1,filter=weights['w2'],strides=[1,1,1,1],padding="VALID")+biases['b2'])
##------ Add your code here: compute the reconstruction of high-resolution image
# conv3 layer with biases and NO relu: 1 filter with size 5 x 5
conv3 = tf.nn.conv2d(input=conv2,filter=weights['w3'],strides=[1,1,1,1],padding="VALID")+biases['b3']
...
"""Read the test image
"""
blurred_image, groudtruth_image = preprocess('images/butterfly_GT.bmp')
